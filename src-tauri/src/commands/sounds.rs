...
pub fn play_sound(sound: Sound, volume: f32) {
    thread::spawn(move || {
        if let Err(e) = play_sound_async(sound, volume) {
            eprintln!("Error playing sound: {}", e);
        }
    });
}

fn play_sound_async(sound: Sound, volume: f32) -> ZammResult<()> {
...
mod tests {
    ...
    fn check_play_sound_sample(file_prefix: &str) {
        ...
        let actual_result = play_sound(request.sound, request.volume);
